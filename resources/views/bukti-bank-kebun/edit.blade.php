@extends('layouts.app')
@section('title', 'Edit Bukti Bank Kebun')
...
<div class="max-w-5xl mx-auto pb-10">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Bukti Bank Kebun</h2>
    </div>
    ...
    <form action="{{ route('bukti-bank-kebun.update', $bukti_bank_kebun->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" id="form-pengajuan">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 md:p-8 border-b border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="no_bukti" value="{{ old('no_bukti', $bukti_bank_kebun->no_bukti) }}" placeholder="Contoh: BKK-2026-001"
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $bukti_bank_kebun->tanggal->format('Y-m-d') ?? date('Y-m-d')) }}" required
                           class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="2"
                              class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">{{ old('keterangan', $bukti_bank_kebun->keterangan) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 md:p-8 bg-gray-50/50">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-bold text-gray-800">Daftar Barang / Keterangan</h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[700px]">
                    ...
                    <tbody id="items-container" class="divide-y divide-gray-100 bg-white">
                        @foreach($bukti_bank_kebun->items as $index => $item)
                        <tr class="item-row">
                            <td class="py-3 px-4 text-sm text-gray-500 text-center row-number">{{ $index + 1 }}</td>
                            <td class="py-3 px-4">
                                <input type="text" name="nama_barang[]" value="{{ old('nama_barang.'.$index, $item->nama_barang) }}" required placeholder="Nama barang" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm">
                            </td>
                            <td class="py-3 px-4">
                                <input type="number" name="qty[]" value="{{ old('qty.'.$index, $item->qty) }}" required min="1" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm input-qty">
                            </td>
                            <td class="py-3 px-4">
                                <input type="number" name="harga_satuan[]" value="{{ old('harga_satuan.'.$index, floatval($item->harga_satuan)) }}" required min="0" placeholder="0" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm input-harga">
                            </td>
                            <td class="py-3 px-4 text-right">
                                <span class="text-sm font-semibold text-gray-800 row-total">0</span>
                            </td>
                            <td class="py-3 px-4 text-center">
                                <button type="button" class="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors btn-remove-item" title="Hapus Baris" {{ count($bukti_bank_kebun->items) == 1 ? 'disabled' : '' }}>
