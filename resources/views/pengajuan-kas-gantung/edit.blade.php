@extends('layouts.app')
@section('title', 'Edit Pengajuan Kas Gantung (A5)')
...
<div class="max-w-5xl mx-auto pb-10">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Pengajuan Kas Gantung (A5)</h2>
    </div>
    ...
    <form action="{{ route('pengajuan-kas-gantung.update', $pengajuan_kas_gantung->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" id="form-pengajuan">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 md:p-8 border-b border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ $pengajuan_kas_gantung->tanggal }}" required
                ...
                <div>
                    ...
                    <input type="text" name="judul_pengajuan" value="{{ $pengajuan_kas_gantung->judul_pengajuan }}" placeholder="Contoh: Belanja Bulanan Basecamp" required
                           class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Pengajuan Untuk Kebutuhan</label>
                    <input type="text" name="pengajuan_kebutuhan" value="{{ $pengajuan_kas_gantung->pengajuan_kebutuhan }}" placeholder="Contoh: KEBUN A"
                           class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="2" placeholder="Tuliskan catatan tambahan di sini..."
                              class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">{{ $pengajuan_kas_gantung->keterangan }}</textarea>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse min-w-[700px]">
                    ...
                    <tbody id="items-container" class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="item-row">
                            ...
                            <td class="py-3 px-4">
                                <input type="text" name="keterangan_pengajuan[]" value="{{ $item->keterangan_pengajuan }}" placeholder="Keterangan..." class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm">
                            </td>
                            <td class="py-3 px-4">
                            ...
                            <td class="py-3 px-4 text-center">
                                <button type="button" class="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors btn-remove-item" title="Hapus Baris" {{ count($pengajuan_kas_gantung->items) == 1 ? 'disabled' : '' }}>
        ...
        <div class="p-6 border-t border-gray-100 bg-white flex justify-end">
            <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-sm transition-all focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                Perbarui Pengajuan Kas Gantung (A5)
            </button>
        </div>
    </form>
</div>
