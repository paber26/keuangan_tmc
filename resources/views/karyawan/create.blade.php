                    ...
                    <div class="space-y-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        @forelse($jabatans as $jabatan)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="jabatans[]" value="{{ $jabatan->id }}" class="w-4 h-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500" {{ (is_array(old('jabatans')) && in_array($jabatan->id, old('jabatans'))) ? 'checked' : '' }}>
