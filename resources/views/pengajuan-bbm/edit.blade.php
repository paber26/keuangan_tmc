...
<div class="w-full pb-10">
    ...
    <form action="{{ route('pengajuan-bbm.update', $pengajuan_bbm->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" id="form-pengajuan">
        @csrf
        @method('PUT')
                        ...
                        <tr class="item-row">
                            ...
                            <td class="py-3 px-4">
                                <select name="tipe_bbm[]" required class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm font-bold input-tipe-bbm">
                                    <option value="Solar" {{ (strtolower($item->tipe_bbm) == 'solar' || stripos($item->uraian, 'solar') !== false) ? 'selected' : '' }}>Solar</option>
                                    <option value="Pertalite" {{ (strtolower($item->tipe_bbm) == 'pertalite' || stripos($item->uraian, 'pertalite') !== false) ? 'selected' : '' }}>Pertalite</option>
                                </select>
                            </td>
                            <td class="py-3 px-4">
                                <input type="number" name="jumlah_liter[]" value="{{ $item->jumlah_liter ?? 0 }}" required min="0" step="0.01" placeholder="0" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm font-bold input-liter">
                            </td>
                            <td class="py-3 px-4">
                                <input type="number" name="harga_per_liter[]" value="{{ $item->harga_per_liter ?? 16000 }}" required min="0" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm font-bold input-harga-liter text-right">
                            </td>
                            <td class="py-3 px-4">
                                <div class="flex items-center gap-1 bg-gray-50 px-3 py-2 rounded border border-gray-100">
                                    ...
                                    <input type="text" readonly class="w-full bg-transparent border-none p-0 focus:ring-0 text-emerald-700 outline-none text-sm font-bold input-total-harga-display text-right" value="{{ number_format(round($item->total_harga), 0, ',', '.') }}">
                                    <input type="hidden" name="total_harga[]" class="input-total-harga" value="{{ round($item->total_harga) }}">
                                </div>
                            </td>
                            <td class="py-3 px-4">
