...
@section('page-subtitle', 'Rekapitulasi Total Pohon Kelapa yang Dipanjat Karyawan')
                ...
                <div class="space-y-6 relative z-10">
                    @php 
                        ...
                        foreach($dataRekap as $d) {
                            if($d['total_pohon'] > $maxPohon) $maxPohon = $d['total_pohon'];
                        }
                    @endphp

                    @foreach($dataRekap as $index => $data)
                        @php
                            $percentage = $maxPohon > 0 ? ($data['total_pohon'] / $maxPohon) * 85 : 0; // Using 85% max width to leave room for the text
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-0">
                            ...
                            <div class="flex-grow flex items-center w-full">
                                ...
                                <span class="ml-3 text-sm font-semibold text-gray-700">{{ number_format($data['total_pohon'], 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
