...
<div class="w-full max-w-4xl mx-auto pb-10">
    <div class="mb-8">
        <a href="{{ $redirect_to }}" class="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-emerald-600 transition-colors mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <select name="kebun_id" required class="searchable-select w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all bg-white">
                        ...
                        @foreach($kebun as $k)
                            <option value="{{ $k->id }}" {{ $dokumentasi->kebun_id == $k->id ? 'selected' : '' }}>{{ $k->virtual_lokasi }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
