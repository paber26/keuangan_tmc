...
<div class="w-full pb-10">
    ...
    <form action="{{ route('pemakaian-bbm.update', $pemakaian_bbm->id) }}" method="POST" enctype="multipart/form-data" class="{{ $formBgClass }}" id="form-pemakaian">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="kebun_id" required class="searchable-select w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all bg-white">
                        ...
                        @foreach($kebun as $k)
                            <option value="{{ $k->id }}" {{ $pemakaian_bbm->kebun_id == $k->id ? 'selected' : '' }}>{{ $k->lokasi }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="karyawan_id" required class="searchable-select w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all bg-white">
                        ...
                        @foreach($karyawan as $k)
                            <option value="{{ $k->id }}" {{ $pemakaian_bbm->karyawan_id == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="2" placeholder="Tuliskan catatan tambahan di sini..."
                              class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">{{ $pemakaian_bbm->keterangan }}</textarea>
                </div>
            </div>
        </div>
                    ...
                    <tbody id="items-container" class="divide-y divide-gray-100 bg-white">
                        @foreach($pemakaian_bbm->items as $index => $item)
                            <tr class="item-row">
                                ...
                                <td class="py-3 px-4">
                                    <input type="text" name="keterangan_pemakaian[]" value="{{ $item->keterangan_pemakaian }}" list="keterangan-list" required placeholder="Cth: Pick Up Grand Max" class="w-full px-3 py-2 rounded border border-gray-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-sm">
                                </td>
                                <td class="py-3 px-4">
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="relative aspect-square rounded-lg overflow-hidden border border-gray-200 shadow-sm group">
                        <img src="{{ Storage::url($img->image_path) }}" alt="Dokumentasi" class="w-full h-full object-cover">
                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-start justify-end p-2">
                            <label class="cursor-pointer flex items-center gap-2 bg-white/90 px-2 py-1 rounded shadow text-sm text-red-600 font-medium hover:bg-red-50">
                                <input type="checkbox" name="remove_images[]" value="{{ $img->id }}" class="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500">
