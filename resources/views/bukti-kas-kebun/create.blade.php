@extends('layouts.app')
@section('page-title', 'Buat Bukti Kas Kebun')
...
<div class="max-w-4xl mx-auto pb-10">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Buat Bukti Pengeluaran Kas Kebun</h2>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('bukti-kas-kebun.store') }}" method="POST">
            @csrf
            <div class="p-6 md:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if(count($pengajuans) > 0)
                            <select name="pengajuan_penggajian_id" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                                <option value="">-- Pilih Form Pengajuan Dana yang Disetujui --</option>
                                @foreach($pengajuans as $p)
                                    <option value="{{ $p->id }}" {{ old('pengajuan_penggajian_id') == $p->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_bukti" value="{{ old('no_bukti') }}" placeholder="Contoh: BKK-001" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    </div>
                </div>
            </div>

            <div class="p-6 border-t border-gray-100 bg-gray-50 flex justify-end">
                <button type="submit" {{ count($pengajuans) == 0 ? 'disabled' : '' }} class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-sm transition-all focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed">
                    Simpan & Buat Bukti
                </button>
            </div>
        </form>
